...
function Assistant({ session, onActionDone }) {
  ...
  const request = async (body) => {
    ...
    const retryNotice = setTimeout(() => setRetrying(true), 16000);
    try {
    ...
    <>
      ...
      {open && <section className="assistant-panel" aria-label="Asistente académico">
        ...
        <div className="assistant-messages">
          ...
          {loading && <p className="assistant-typing" aria-live="polite">{retrying ? "La respuesta está tardando un poco más de lo habitual..." : "Pensando..."}</p>}
        </div>
